...
void fft1(std::vector<std::complex<FLOAT >>& a, bool inverse = false) {
    ...
    FLOAT  angle = (inverse ? 1.0 : -1.0) * 2.0 * 3.141592 / n;
    std::complex<FLOAT > w(1.0, 0.0), wn(cos(angle), sin(angle));

    for (int i = 0; i < n / 2; ++i) {
        std::complex<FLOAT > t = w * odd[i];
        a[i] = even[i] + t;
        a[i + n / 2] = even[i] - t;
        w *= wn;
    }
...
std::vector<FLOAT > ifft(const std::vector<std::complex<FLOAT >>& input, bool symmetric) {
    std::vector<std::complex<FLOAT >> a = input;
    int n = a.size();
