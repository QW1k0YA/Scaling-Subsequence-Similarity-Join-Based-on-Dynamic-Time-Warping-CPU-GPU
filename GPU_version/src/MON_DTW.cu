...
__device__ std::size_t cap_start_index_to_window(std::size_t index, std::size_t window){
    if(index>window){ return index-window; } else { return 0; }
}

__device__ std::size_t cap_stop_index_to_window_or_end(std::size_t index, std::size_t window, std::size_t end){
    
    if(window<end && index+1<end-window){ return index + window + 1; } else { return end; }
}

inline std::size_t cap_start_index_to_window_host(std::size_t index, std::size_t window){
    if(index>window){ return index-window; } else { return 0; }
}

inline std::size_t cap_stop_index_to_window_or_end_host(std::size_t index, std::size_t window, std::size_t end){
...
__device__ FLOAT MON_dtw(const FLOAT *lines, const FLOAT *cols, int l, int w, FLOAT bsf, FLOAT *buffers, int tid)
{
    ...
    for(size_t i=0; i<nblines; ++i) {
        ...
        my_swap(c, p);
        if(i+w < nbcols){ UB = bsf; }
        const FLOAT li = lines[i];
...
/**
 * Host implementation of the monotonic DTW algorithm with Sakoe-Chiba band constraint.
 * This function computes the DTW distance between two sequences with a warping window
 * constraint, using pruning based on best-so-far (bsf) value and cost buffer (cb).
 *
 * @param lines: First sequence (query)
 * @param cols: Second sequence (reference)
 * @param cb: Cost buffer from previous lower bound computation
 * @param l: Length of sequences
 * @param w: Warping window size
 * @param bsf: Best-so-far distance for pruning
 * @return FLOAT: DTW distance between the two sequences, or infinity if pruned
 */
FLOAT MON_dtw_host(
        const FLOAT*  lines,
        const FLOAT*  cols,
...
)
{
    ...
    std::vector<FLOAT> buffers_v((1+nbcols) * 2, POSITIVE_INFINITY);
    FLOAT *buffers = buffers_v.data();
    size_t c{1}, p{nbcols+2};                 
    ...
    for(size_t i=0; i<nblines; ++i) {
        ...
        const FLOAT li = lines[i];
        const std::size_t jStop = cap_stop_index_to_window_or_end_host(i, w, nbcols);
        
        const std::size_t jStart = std::max(cap_start_index_to_window_host(i, w), next_start);
