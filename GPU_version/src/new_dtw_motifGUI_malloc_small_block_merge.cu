...
void new_dtw_motifGUI_malloc_small_block_merge(const vector<FLOAT> &a, int subseqLen, int maxwarp, const vector<FLOAT> &mp_ed,
                             const vector<int> &mp_ed_index, const char *file)
{
    ...
    int *d_prefix_sum;
    cudaError_t err = cudaMalloc(&d_prefix_sum, GRID_SIZE * sizeof(int)); CUERR
    if (err != cudaSuccess) {
    ...
    int *d_prefix_sum_1;
    int *d_prefix_sum_2;
    cudaMalloc(&d_prefix_sum_1, GRID_SIZE * sizeof(int));
    cudaMalloc(&d_prefix_sum_2, GRID_SIZE * sizeof(int));


    int *d_num_inclusive;
    cudaMalloc(&d_num_inclusive, THREADS_NUM * sizeof(int)); CUERR
    int *d_num_inclusive_1;
    cudaMalloc(&d_num_inclusive_1, THREADS_NUM * sizeof(int)); CUERR
    int *d_num_inclusive_2;
    cudaMalloc(&d_num_inclusive_2, THREADS_NUM * sizeof(int)); CUERR

    int init_d_num_of_dtw = 0;

    int *h_num_of_dtw;
    h_num_of_dtw = (int*) malloc(3*sizeof(int));
    h_num_of_dtw[0] = 0;
    h_num_of_dtw[1] = 0;
    h_num_of_dtw[2] = 0;
    ...
    bool* d_lb_vector_1;
    cudaMalloc(&d_lb_vector_1, num_threads * STEP_LENGTH * sizeof(bool));
    bool* d_lb_vector_2;
    cudaMalloc(&d_lb_vector_2, num_threads * STEP_LENGTH * sizeof(bool));
    ...
    cudaMemcpy(d_lb_vector_1, h_lb_vector, num_threads * STEP_LENGTH * sizeof(bool), cudaMemcpyHostToDevice);
    cudaMemcpy(d_lb_vector_2, h_lb_vector, num_threads * STEP_LENGTH * sizeof(bool), cudaMemcpyHostToDevice);
    int* d_indices;
    cudaMalloc(&d_indices, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    int* d_indices_1;
    cudaMalloc(&d_indices_1, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    int* d_indices_2;
    cudaMalloc(&d_indices_2, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    int* d_diag;
    cudaMalloc(&d_diag, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    int* d_diag_1;
    cudaMalloc(&d_diag_1, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    int* d_diag_2;
    cudaMalloc(&d_diag_2, num_threads * STEP_LENGTH * sizeof(int)); CUERR
    ...
    cudaMemcpy(d_diag_1,  h_indices, num_threads * STEP_LENGTH * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_diag_2,  h_indices, num_threads * STEP_LENGTH * sizeof(int), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(d_indices_1,  h_indices, num_threads * STEP_LENGTH * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_indices_2,  h_indices, num_threads * STEP_LENGTH * sizeof(int), cudaMemcpyHostToDevice);

    FLOAT*  d_lb_vector_new;
    cudaMalloc(&d_lb_vector_new, num_threads * STEP_LENGTH * sizeof(FLOAT));
    FLOAT*  d_lb_vector_new_1;
    cudaMalloc(&d_lb_vector_new_1, num_threads * STEP_LENGTH * sizeof(FLOAT));
    FLOAT*  d_lb_vector_new_2;
    cudaMalloc(&d_lb_vector_new_2, num_threads * STEP_LENGTH * sizeof(FLOAT));
    ...
    FLOAT*  d_bsf_global_1 = nullptr;
    FLOAT*  d_bsf_global_2 = nullptr;

    cudaMalloc(&d_bsf_global, BSF_POOL * sizeof(FLOAT));
    cudaMemcpy(d_bsf_global, h_bsf, BSF_POOL * sizeof(FLOAT), cudaMemcpyHostToDevice);
    cudaMalloc(&d_bsf_global_1, BSF_POOL * sizeof(FLOAT));
    cudaMemcpy(d_bsf_global_1, h_bsf, BSF_POOL * sizeof(FLOAT), cudaMemcpyHostToDevice);
    cudaMalloc(&d_bsf_global_2, BSF_POOL * sizeof(FLOAT));
    cudaMemcpy(d_bsf_global_2, h_bsf, BSF_POOL * sizeof(FLOAT), cudaMemcpyHostToDevice);
    ...
    for(int t_index = 0;t_index < t_for_cnt;t_index ++)
    {
        ...
        for (int diag = minlag + 1; diag <= subcount-d_start_pos; diag += THREADS_NUM)
        {
            tasks.push_back({d_start_pos,d_end_pos,diag});
        }
    }
    uint tasks_len = tasks.size();

    uint tail = tasks_len%3;
    uint tasks_realen = tasks_len-tail;

    int task_index_1;
    int task_index_2;

    for (int task_index = 0; task_index < tasks_realen; task_index+=3)
    {
        task_index_1 = task_index + 1;
        task_index_2 = task_index + 2;
        FLOAT t1 = clock();
        GLOBAL_DIAG_3TASKS<<<GRID_SIZE, BLOCK_SIZE>>>
            (minlag,subcount,subseqLen,len,warpmax,d_a,d_mu,d_sumU_sumL,d_invsig,d_norm_U_plus_norm_L_trans,d_dr_bwdU_plus_dr_bwdL,
            d_dc_bwd,d_dr_fwdU_plus_dr_fwdL,d_dc_fwd,d_UTS,d_LTS,d_UTS_global,d_LTS_global,d_MASK_global,d_TS2,d_sumMASK_global,
            d_sumU_sumL_global,d_dr_bwdU_plus_dr_bwdL_global,d_dr_fwdU_plus_dr_fwdL_global,d_dc_bwd_global,d_dc_fwd_global,d_dr_bwdMASK_global,
            d_dr_fwdMASK_global,d_dc_bwdTS2_global,d_dc_fwdTS2_global,d_DUL2_global,d_DUL_global,d_norm_U_plus_norm_L_global,
            d_my_subs,d_shared_special_vector,d_bsf_global, d_DUL_fast,d_DUL2_fast,tasks[task_index],tasks[task_index_1],tasks[task_index_2],d_lb_vector,d_lb_vector_new,
            d_lb_vector_1,d_lb_vector_new_1,d_lb_vector_2,d_lb_vector_new_2 );
        ...
        if (syncErr != cudaSuccess) {
            ...
            printf("diag = %d st = %d end = %d\n",tasks[task_index].diag,
                tasks[task_index].start_pos,tasks[task_index].end_pos);
        }
        ...
        calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE>>>(d_lb_vector, subcount,
                                                                tasks[task_index].start_pos, tasks[task_index].end_pos,
                                                                tasks[task_index].diag, d_prefix_sum, d_num_inclusive);

        {
            calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE>>>(d_lb_vector_1, subcount,
            tasks[task_index_1].start_pos, tasks[task_index_1].end_pos,
            tasks[task_index_1].diag, d_prefix_sum_1, d_num_inclusive_1);

        }

        {
            calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE>>>(d_lb_vector_2, subcount,
           tasks[task_index_2].start_pos, tasks[task_index_2].end_pos,
           tasks[task_index_2].diag, d_prefix_sum_2, d_num_inclusive_2);
        }
        cudaDeviceSynchronize();

        device_prefix_sum_block<<<1, GRID_SIZE>>>(d_prefix_sum);
        device_prefix_sum_block<<<1, GRID_SIZE>>>(d_prefix_sum_1);
        device_prefix_sum_block<<<1, GRID_SIZE>>>(d_prefix_sum_2);
        ...
        {
            collect_indices<<<GRID_SIZE, BLOCK_SIZE>>>(d_diag_1, d_indices_1, d_lb_vector_1, subcount,
            tasks[task_index_1].start_pos, tasks[task_index_1].end_pos,
                       tasks[task_index_1].diag,  d_prefix_sum_1, d_num_inclusive_1);
        }
        ...
        {
            collect_indices<<<GRID_SIZE, BLOCK_SIZE>>>(d_diag_2, d_indices_2, d_lb_vector_2, subcount,
            tasks[task_index_2].start_pos, tasks[task_index_2].end_pos,
                       tasks[task_index_2].diag,  d_prefix_sum_2, d_num_inclusive_2);
        }
        ...
        cudaMemcpy(h_num_of_dtw+1, &d_prefix_sum_1[GRID_SIZE - 1], sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_num_of_dtw+2, &d_prefix_sum_2[GRID_SIZE - 1], sizeof(int), cudaMemcpyDeviceToHost);

        dtw_cnt+= (h_num_of_dtw[0]+h_num_of_dtw[1]+h_num_of_dtw[2]);
        ...
        if(subseqLen < 1024)
        {
            ...
            if (task_index_1 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram<<<h_num_of_dtw[1], 32, DTW_SHARED_MEM_SIZE>>>
        (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
         warpmax, d_indices_1, d_diag_1);
            if (task_index_2 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram<<<h_num_of_dtw[2], 32, DTW_SHARED_MEM_SIZE>>>
        (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
         warpmax, d_indices_2, d_diag_2);
        ...
        else
        {
            ...
            if (task_index_1 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram_without_shared_memory_and_nomalized<<<h_num_of_dtw[1], 32>>>
        (d_my_subs, d_UTS, d_LTS, d_mu, d_invsig, subseqLen, d_bsf_global_1, subcount,
         warpmax, d_indices_1, d_diag_1, bl_size);
            if (task_index_2 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram_without_shared_memory_and_nomalized<<<h_num_of_dtw[2], 32>>>
        (d_my_subs, d_UTS, d_LTS, d_mu, d_invsig, subseqLen, d_bsf_global_2, subcount,
         warpmax, d_indices_2, d_diag_2, bl_size);
        ...
        syn_bsfs_3<<<1,BSF_POOL>>>(d_bsf_global,d_bsf_global_1,d_bsf_global_2);
        cudaDeviceSynchronize();
    ...
    if (tail)
    {
        int task_index = tasks_realen;

    {
        task_index_1 = task_index + 1;
        FLOAT t1 = clock();
        GLOBAL_DIAG<<<GRID_SIZE, BLOCK_SIZE,0,stream0>>>
            (minlag,subcount,subseqLen,len,warpmax,d_a,d_mu,d_sumU_sumL,d_invsig,d_norm_U_plus_norm_L_trans,d_dr_bwdU_plus_dr_bwdL,
            d_dc_bwd,d_dr_fwdU_plus_dr_fwdL,d_dc_fwd,d_UTS,d_LTS,d_UTS_global,d_LTS_global,d_MASK_global,d_TS2,d_lb_vector,d_lb_vector_new,d_sumMASK_global,
            d_sumU_sumL_global,d_dr_bwdU_plus_dr_bwdL_global,d_dr_fwdU_plus_dr_fwdL_global,d_dc_bwd_global,d_dc_fwd_global,d_dr_bwdMASK_global,
            d_dr_fwdMASK_global,d_dc_bwdTS2_global,d_dc_fwdTS2_global,d_DUL2_global,d_DUL_global,d_norm_U_plus_norm_L_global,
            d_my_subs,d_shared_special_vector,d_bsf_global, d_DUL_fast,d_DUL2_fast, tasks[task_index].diag, tasks[task_index].start_pos, tasks[task_index].end_pos);
        

        if (task_index_1 < tasks_len)
        {
            GLOBAL_DIAG<<<GRID_SIZE, BLOCK_SIZE,0,stream1>>>
                (minlag,subcount,subseqLen,len,warpmax,d_a,d_mu,d_sumU_sumL,d_invsig,d_norm_U_plus_norm_L_trans,d_dr_bwdU_plus_dr_bwdL,
                d_dc_bwd,d_dr_fwdU_plus_dr_fwdL,d_dc_fwd,d_UTS,d_LTS,d_UTS_global,d_LTS_global,d_MASK_global,d_TS2,d_lb_vector_1,d_lb_vector_new_1,d_sumMASK_global,
                d_sumU_sumL_global,d_dr_bwdU_plus_dr_bwdL_global,d_dr_fwdU_plus_dr_fwdL_global,d_dc_bwd_global,d_dc_fwd_global,d_dr_bwdMASK_global,
                d_dr_fwdMASK_global,d_dc_bwdTS2_global,d_dc_fwdTS2_global,d_DUL2_global,d_DUL_global,d_norm_U_plus_norm_L_global,
                d_my_subs,d_shared_special_vector,d_bsf_global, d_DUL_fast,d_DUL2_fast,  tasks[task_index_1].diag, tasks[task_index_1].start_pos, tasks[task_index_1].end_pos);
            
        }


        calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE,0,stream0>>>(d_lb_vector, subcount,
                                                                tasks[task_index].start_pos, tasks[task_index].end_pos,
                                                                tasks[task_index].diag, d_prefix_sum, d_num_inclusive);
        if (task_index_1 < tasks_len)
        {
            calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE,0,stream1>>>(d_lb_vector_1, subcount,
            tasks[task_index_1].start_pos, tasks[task_index_1].end_pos,
            tasks[task_index_1].diag, d_prefix_sum_1, d_num_inclusive_1);
        ...
        device_prefix_sum_block<<<1, GRID_SIZE,0,stream0>>>(d_prefix_sum);
         if (task_index_1 < tasks_len) device_prefix_sum_block<<<1, GRID_SIZE,0,stream1>>>(d_prefix_sum_1);

        

        collect_indices<<<GRID_SIZE, BLOCK_SIZE,0,stream0>>>(d_diag, d_indices, d_lb_vector, subcount,
        tasks[task_index].start_pos, tasks[task_index].end_pos,
        tasks[task_index].diag,  d_prefix_sum, d_num_inclusive);
        if (task_index_1 < tasks_len)
        {
            collect_indices<<<GRID_SIZE, BLOCK_SIZE,0,stream1>>>(d_diag_1, d_indices_1, d_lb_vector_1, subcount,
            tasks[task_index_1].start_pos, tasks[task_index_1].end_pos,
                       tasks[task_index_1].diag,  d_prefix_sum_1, d_num_inclusive_1);
        }
        ...
        dtw_cnt+= (h_num_of_dtw[0]+h_num_of_dtw[1]);
        cudaError_t syncErr = cudaDeviceSynchronize();
        if (syncErr != cudaSuccess) {
            printf("fail in collect %s\n", cudaGetErrorString(syncErr));
        }
        ...
        if(subseqLen < 1024)
        {
            ...
            process_keogh_and_dtw_kernel_for_a_Parallelogram<<<h_num_of_dtw[0], 32, DTW_SHARED_MEM_SIZE,stream0>>>
                    (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
                     warpmax, d_indices, d_diag);
            if (task_index_1 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram<<<h_num_of_dtw[1], 32, DTW_SHARED_MEM_SIZE,stream1>>>
        (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
         warpmax, d_indices_1, d_diag_1);
        ...
        {
            process_keogh_and_dtw_kernel_for_a_Parallelogram_without_shared_memory_and_nomalized<<<h_num_of_dtw[0], 32,0,stream0>>>
                    (d_my_subs, d_UTS, d_LTS, d_mu, d_invsig, subseqLen, d_bsf_global, subcount,
                     warpmax, d_indices, d_diag, bl_size);
            if (task_index_1 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram_without_shared_memory_and_nomalized<<<h_num_of_dtw[1], 32,0,stream1>>>
        (d_my_subs, d_UTS, d_LTS, d_mu, d_invsig, subseqLen, d_bsf_global_1, subcount,
         warpmax, d_indices_1, d_diag_1, bl_size);
        }
        syncErr = cudaDeviceSynchronize();
    ...
    cudaFree(d_bsf_global_1);
    cudaFree(d_bsf_global_2);
    ...
    cudaFree(d_lb_vector_1);
    cudaFree(d_lb_vector_2);
    cudaFree(d_lb_vector_new);
    cudaFree(d_lb_vector_new_1);
    cudaFree(d_lb_vector_new_2);
    cudaFree(d_indices);
    cudaFree(d_indices_1);
    cudaFree(d_indices_2);
    cudaFree(d_diag);
    cudaFree(d_diag_1);
    cudaFree(d_diag_2);
    cudaFree(d_prefix_sum);
    cudaFree(d_prefix_sum_1);
    cudaFree(d_prefix_sum_2);

    cudaFree(d_num_inclusive);
    cudaFree(d_num_inclusive_1);
    cudaFree(d_num_inclusive_2);
