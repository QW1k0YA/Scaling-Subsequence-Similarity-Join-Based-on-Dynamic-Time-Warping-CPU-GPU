...
/**
 * GPU-accelerated DTW motif discovery algorithm with memory-efficient block processing.
 * This function implements a GPU-based approach for discovering the most similar pair of
 * subsequences in a time series using Dynamic Time Warping distance with multiple
 * optimization techniques and memory management strategies.
 *
 * @param a: Input time series data
 * @param subseqLen: Length of subsequences to compare
 * @param maxwarp: Maximum warping window size for DTW
 * @param mp_ed: Matrix profile based on Euclidean distance (used for initialization)
 * @param mp_ed_index: Index array corresponding to the matrix profile
 * @param file: Path to output timing results
 */
void new_dtw_motifGUI_malloc_small_block(const vector<FLOAT> &a, int subseqLen, int maxwarp, const vector<FLOAT> &mp_ed,
                             const vector<int> &mp_ed_index, const char *file)
{
    ...
    for (int diag = minlag + 1; diag <= subcount; diag += THREADS_NUM)
    {
        ...
        {
            int d_start_pos = t_index * step;
            
            if (diag <= subcount - d_start_pos)
            {
                int d_end_pos = d_start_pos + step;
    ...
    for (int task_index = 0; task_index < tasks_len; task_index+=3)
    {
        ...
        if (task_index_2 < tasks_len)
        {
            GLOBAL_DIAG<<<GRID_SIZE, BLOCK_SIZE,0,stream2>>>
                (minlag,subcount,subseqLen,len,warpmax,d_a,d_mu,d_sumU_sumL,d_invsig,d_norm_U_plus_norm_L_trans,d_dr_bwdU_plus_dr_bwdL,
                d_dc_bwd,d_dr_fwdU_plus_dr_fwdL,d_dc_fwd,d_UTS,d_LTS,d_UTS_global,d_LTS_global,d_MASK_global,d_TS2,d_lb_vector_2,d_lb_vector_new_2,d_sumMASK_global,
                d_sumU_sumL_global,d_dr_bwdU_plus_dr_bwdL_global,d_dr_fwdU_plus_dr_fwdL_global,d_dc_bwd_global,d_dc_fwd_global,d_dr_bwdMASK_global,
                d_dr_fwdMASK_global,d_dc_bwdTS2_global,d_dc_fwdTS2_global,d_DUL2_global,d_DUL_global,d_norm_U_plus_norm_L_global,
                d_my_subs,d_shared_special_vector,d_bsf_global, d_DUL_fast,d_DUL2_fast,
                tasks[task_index_2].diag, tasks[task_index_2].start_pos, tasks[task_index_2].end_pos);
        ...
        if (task_index_2 < tasks_len)
        {
            calculate_num_for_each_block<<<GRID_SIZE, BLOCK_SIZE,0,stream2>>>(d_lb_vector_2, subcount,
        ...
        device_prefix_sum_block<<<1, GRID_SIZE,0,stream0>>>(d_prefix_sum);
         ...
         if (task_index_2 < tasks_len) device_prefix_sum_block<<<1, GRID_SIZE,0,stream2>>>(d_prefix_sum_2);
        ...
        if (task_index_2 < tasks_len)
        {
            collect_indices<<<GRID_SIZE, BLOCK_SIZE,0,stream2>>>(d_diag_2, d_indices_2, d_lb_vector_2, subcount,
            tasks[task_index_2].start_pos, tasks[task_index_2].end_pos,
                       tasks[task_index_2].diag,  d_prefix_sum_2, d_num_inclusive_2);
        }
        ...
        (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
         warpmax, d_indices_1, d_diag_1);
            if (task_index_2 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram<<<h_num_of_dtw[2], 32, DTW_SHARED_MEM_SIZE,stream2>>>
        (d_my_subs, d_subs_L, d_subs_U, subseqLen, d_bsf_global, subcount,
        ...
        (d_my_subs, d_UTS, d_LTS, d_mu, d_invsig, subseqLen, d_bsf_global_1, subcount,
         warpmax, d_indices_1, d_diag_1, bl_size);
            if (task_index_2 < tasks_len) process_keogh_and_dtw_kernel_for_a_Parallelogram_without_shared_memory_and_nomalized<<<h_num_of_dtw[2], 32,0,stream2>>>
