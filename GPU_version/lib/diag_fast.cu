...
/**
 * Device function for fast diagonal segment computation with lower bound estimation.
 * This function performs the first level of pruning by computing a fast approximation
 * of the DTW distance using precomputed values and checking against the best-so-far
 * threshold to eliminate unnecessary computations.
 *
 * @param ts: Time series data
 * @param subseqlen: Length of subsequences
 * @param diag_ID: Diagonal ID being processed
...
__device__ void diag_fast_segment_shared(const FLOAT *ts, int subseqlen, int diag_ID, const FLOAT *UTS, const FLOAT *LTS,
                                         const FLOAT *mu, const FLOAT *sumU_sumL, const FLOAT *invsig,
                                         const FLOAT *norm_U_plus_norm_L_trans, bool *lb_vector,
                                         const FLOAT *dr_bwdU_plus_dr_bwdL, const FLOAT *dc_bwd,
                                         const FLOAT *dr_fwdU_plus_dr_fwdL, const FLOAT *dc_fwd, FLOAT &cnt,
    ...
    for (int low_index = start_pos + 1; low_index < end_pos; low_index++) {
        ...
        cov_U_plus_cov_L_fir = cov_U_plus_cov_L_fir -
                               dr_bwdU_plus_dr_bwdL_low_index[lb_pos] * dc_bwd[high_index] + dr_fwdU_plus_dr_fwdL_low_index[lb_pos] * dc_fwd[high_index];

        cov_U_plus_cov_L_sec = cov_U_plus_cov_L_sec -
                               dr_bwdU_plus_dr_bwdL[high_index] * dc_bwd_low_index[lb_pos] + dr_fwdU_plus_dr_fwdL[high_index] * dc_fwd_low_index[lb_pos];
