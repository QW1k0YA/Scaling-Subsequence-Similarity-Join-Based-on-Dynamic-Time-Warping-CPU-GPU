...
/**
 * Device function for global diagonal masking with advanced lower bound computation.
 * This function performs the second level of pruning by computing a more accurate
 * lower bound using global masks and additional optimizations to further reduce
 * the number of full DTW computations needed.
 *
 * @param ts: Time series data
 * @param subseqlen: Length of subsequences
 * @param diagID: Diagonal ID being processed
 * @param lb_vector: Boolean vector for storing lower bound test results
 ...
 * @param MASK: Global mask for pruning
 * @param TS2: Squared time series values
 * @param sumMASK: Sum of mask values
 ...
 * @param dr_bwdMASK: Backward differences of mask
 * @param dr_fwdMASK: Forward differences of mask
 * @param dc_bwdTS2: Backward differences of squared time series
 * @param dc_fwdTS2: Forward differences of squared time series
 * @param DUL2: Squared distance between upper and lower envelopes
 * @param lb_vector_new: Updated lower bound values
 * @param bsf: Best-so-far distance threshold
 * @param subs: Array of normalized subsequences
 * @param special_shared_vector: Special shared vectors for optimization
 * @param cnt: Counter for pruned elements
 * @param DUL: Distance between upper and lower envelopes
...
__device__ void
diag_mask_global_shared_mem(const FLOAT *ts, int subseqlen, int diagID, bool *lb_vector, const FLOAT *mu, const FLOAT *UTS,
                 const FLOAT *LTS, const FLOAT *MASK, const FLOAT *TS2, const FLOAT *sumMASK,
                 const FLOAT *invsig, const FLOAT *sumU_sumL, const FLOAT *dr_bwdU_plus_dr_bwdL,
                 const FLOAT *dr_fwdU_plus_dr_fwdL, const FLOAT *dc_bwd, const FLOAT *dc_fwd,
                 const FLOAT *dr_bwdMASK, const FLOAT *dr_fwdMASK, const FLOAT *dc_bwdTS2,
                 ...
                 FLOAT **special_shared_vector, FLOAT &cnt, const FLOAT *DUL,
                 const FLOAT *norm_U_plus_norm_L_global, int start_pos, int end_pos,
                                         const FLOAT* dr_bwdU_plus_dr_bwdL_low_index,
                                         const FLOAT* dc_bwd_low_index,
                                         const FLOAT* dr_fwdU_plus_dr_fwdL_low_index,
                                         const FLOAT* dc_fwd_low_index)
{
    FLOAT  M_NORM,t,dist,DUL_value;
    ...
    for (int low_index = start_pos + 1; low_index < end_pos; low_index++) {
        ...
        cov_U_plus_cov_L_fir = cov_U_plus_cov_L_fir - dr_bwdU_plus_dr_bwdL_low_index[lb_pos] * dc_bwd[high_index ] +
                               (dr_fwdU_plus_dr_fwdL_low_index[lb_pos ]) * dc_fwd[high_index];
        dot_TS_M_fir = dot_TS_M_fir - dr_bwdMASK[low_index] * dc_bwd[high_index] + dr_fwdMASK[low_index] * dc_fwd[high_index];
        dot_TS2_M_fir = dot_TS2_M_fir - dr_bwdMASK[low_index] * dc_bwdTS2[high_index] + dr_fwdMASK[low_index] * dc_fwdTS2[high_index];

        cov_U_plus_cov_L_sec = cov_U_plus_cov_L_sec - dr_bwdU_plus_dr_bwdL[high_index] * dc_bwd_low_index[lb_pos] +
                               (dr_fwdU_plus_dr_fwdL[high_index]) * dc_fwd_low_index[lb_pos];
        dot_TS_M_sec = dot_TS_M_sec - dr_bwdMASK[high_index] * dc_bwd_low_index[lb_pos] + dr_fwdMASK[high_index] * dc_fwd_low_index[lb_pos];
        dot_TS2_M_sec = dot_TS2_M_sec - dr_bwdMASK[high_index] * dc_bwdTS2[low_index] + dr_fwdMASK[high_index] * dc_fwdTS2[low_index];
