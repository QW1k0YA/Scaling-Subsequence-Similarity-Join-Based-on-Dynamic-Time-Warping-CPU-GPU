...
/**
 * Computes the Dynamic Time Warping (DTW) distance between two time series sequences A and B
 * with early abandoning based on a best-so-far threshold.
 *
 * @param A: First time series sequence
 * @param B: Second time series sequence
 * @param cb: Cumulative boundary values for early abandoning
 * @param m: Length of the sequences
 * @param r: Warping window size (band radius)
 * @param best_so_far: Current best DTW distance threshold for early abandoning
 * @return: Pointer to an array containing [DTW distance, pruning counter]
 */
...
/**
 * Computes the Dynamic Time Warping (DTW) distance between two time series sequences A and B
 * with early abandoning based on a best-so-far threshold (bsf).
 *
 * @param A: First time series sequence
 * @param B: Second time series sequence
 * @param cb: Cumulative boundary values for early abandoning
 * @param m: Length of the sequences
 * @param r: Warping window size (band radius)
 * @param bsf: Best-so-far threshold for early abandoning (default: infinity)
 * @return: Pointer to an array containing [DTW distance, pruning counter]
 */
...
/**
 * Updated version of DTW computation with early abandoning based on best-so-far threshold.
 *
 * @param x: First time series sequence
 * @param y: Second time series sequence
 * @param warpmax: Maximum warping window size
 * @param best_so_far: Best-so-far threshold for early abandoning (default: infinity)
 * @return: Pointer to an array containing [DTW distance, pruning counter]
 */
