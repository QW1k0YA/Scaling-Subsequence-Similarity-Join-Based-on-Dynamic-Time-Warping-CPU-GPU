...
/**
 * Local diagonal-based masking for DTW motif discovery (upper part).
 * This function applies a local masking technique to compute lower bounds for DTW distances
 * along a diagonal, focusing on the upper portion of the matrix profile.
 *
 * @param ts: Input time series
 * @param minlag: Minimum lag for diagonal computation
 * @param subseqlen: Length of subsequences
 * @param bsf: Best-so-far distance threshold
 * @param diagID: Diagonal ID in the matrix profile
 * @param lb_vector: Boolean vector indicating which pairs to prune
 * @param count_table: Count table for cumulative distribution
 * @param pos_UU: Position indices for upper envelope
 * @param pos_LL: Position indices for lower envelope
 * @param mu: Mean values for normalization
 * @param UTS_: Upper envelope of the time series
 * @param LTS_: Lower envelope of the time series
 * @param special_shared_vector: Shared vectors for intermediate calculations
 * @param cnt: Counter for pruning events
 * @param sumU2_sumL2: Sum of squared upper and lower envelope values
 * @param sumU_sumL: Sum of upper and lower envelope values
 * @param sumMASK: Sum of mask values for normalization
 * @param norm_U_plus_norm_L: Normalized upper and lower envelope terms
 * @param raw_DIFF_UL: Raw difference between upper and lower envelopes
 * @param raw_DIFF_UL2: Squared raw difference between upper and lower envelopes
 * @param raw_DIFF_UL2_temp: Temporary squared difference vector
 * @param DUL2_raw: Raw squared distance values for upper and lower bounds
 * @param DUL2: Squared distance values for upper and lower bounds
 * @param TS2: Squared time series values
 * @param alpha: Alpha parameter for threshold adjustment
 * @param invsig: Inverse of standard deviation for normalization
 * @param invsig_2: Squared inverse of standard deviation
 * @param diag_bias: Diagonal bias offset
 */
void
                   ...
                   const vector<double> &invsig_2, int diag_bias)
{
    ...
    vector<DOUBLE> normal_DIFF(len - diag_bias,0.0);
    size_t posLL,posUU;
    size_t pos;
    DOUBLE diff2,diff_ratio;
    
    for(size_t i = 0;i < len - diag_bias;i++)
    {
        pos = i + diagID -1;
        if(pos >= len)
        {
    ...
    substractvector(UTS_,LTS_,raw_DIFF_UL,len - diag_bias);
    elementWiseMultiply(raw_DIFF_UL,raw_DIFF_UL,raw_DIFF_UL2_temp,len - diag_bias);
    ...
    vector<DOUBLE> UTS = elementWiseMultiply_p(UTS_.data(),MASK.data(),len - diag_bias);
    vector<DOUBLE> LTS = elementWiseMultiply_p(LTS_.data(),MASK.data(),len - diag_bias);

    dr_bwdU_plus_dr_bwdL_less = plusvector_p(UTS.data() + BIAS, LTS.data() + BIAS,len  -1 - diag_bias);
    
    dr_fwdU_plus_dr_fwdL = plusvector_p(UTS.data() + subseqlen - BIAS - 1, LTS.data() + subseqlen - BIAS - 1,len - subseqlen + 1 - diag_bias);
    ...
    for(i = 0;i < subseqlen - DOUBLE_BIAS;i++)
    { 
    ...
    for(i = 0;i < subcount - DOUBLE_BIAS - diag_bias;i++)
    {
    ...
    for(size_t row = 0;row < subcount - diag_bias;row++)
    {
        norm_U_plus_norm_L[row] = (sumU2_sumL2[row] - 2*sumU_sumL[row]*mu[row]
                                     + 2*sumMASK[row]*mu[row]*mu[row])*invsig[row]*invsig[row];
    }

    elementWiseMultiply(raw_DIFF_UL2_temp,MASK,raw_DIFF_UL2,len - diag_bias);
    movsum_p(raw_DIFF_UL2.data() + BIAS,len - diag_bias - DOUBLE_BIAS, subseqlen - 1 - DOUBLE_BIAS,DUL2_raw);
    elementWiseMultiply(DUL2_raw,invsig_2,DUL2,len - diag_bias);
    ...
    if(!lb_vector[0])
    {
        ...
        if(lb > bsf || (lb*lb + special_shared_vector[row][0] + special_shared_vector[row][1] > bsf*bsf))
        {
            lb_vector[row] = true;
            cnt++;
        }
    ...
    for (low_index = 1; low_index < temp_1; low_index++) {
        ...
        cov_U_plus_cov_L_fir = cov_U_plus_cov_L_fir - dr_bwdU_plus_dr_bwdL_less[low_index_less] * dc_bwd_less[high_index_less ] +
                           dr_fwdU_plus_dr_fwdL[low_index] * dc_fwd[high_index ];
        dot_TS_M_fir = dot_TS_M_fir - dr_bwdMASK_less[low_index_less] * dc_bwd_less[high_index_less] + dr_fwdMASK[low_index] * dc_fwd[high_index];
        dot_TS2_M_fir= dot_TS2_M_fir - dr_bwdMASK_less[low_index_less] * dc_bwdTS2_less[high_index_less] + dr_fwdMASK[low_index] * dc_fwdTS2[high_index];
        ...
        dist = 2 * M_NORM - 2 * t + norm_U_plus_norm_L[low_index];
        ...
        {
            DUL_value = sqrt(MAX(DUL2[low_index],0));
            lb = 0.5*(sqrt(2 * dist - DUL2[low_index]) - DUL_value);
