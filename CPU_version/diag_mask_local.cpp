                ...
                double alpha, const vector<double> &invsig, const vector<double> &invsig_2)
{
    ...
    substractvector(UTS_,LTS_,raw_DIFF_UL);
    elementWiseMultiply(raw_DIFF_UL,raw_DIFF_UL,raw_DIFF_UL2_temp);
    ...
    dr_bwdU_plus_dr_bwdL_less = plusvector_p(UTS.data() + BIAS, LTS.data() + BIAS,UTS.size()-1);
    
    dr_fwdU_plus_dr_fwdL = plusvector_p(UTS.data() + subseqlen - BIAS - 1, LTS.data() + subseqlen - BIAS - 1,UTS.size() - subseqlen + 1);
    ...
    for(i = 0;i < subcount - DOUBLE_BIAS;i++)
    {
    ...
    for(size_t row = 0;row < subcount;row++)
    {
        norm_U_plus_norm_L[row] = (sumU2_sumL2[row] - 2*sumU_sumL[row]*mu[row]
                                     + 2*sumMASK[row]*mu[row]*mu[row])*invsig[row]*invsig[row];
    }

    elementWiseMultiply(raw_DIFF_UL2_temp,MASK,raw_DIFF_UL2);
    movsum_p(raw_DIFF_UL2.data() + BIAS,raw_DIFF_UL2.size() - DOUBLE_BIAS, subseqlen - 1 - DOUBLE_BIAS,DUL2_raw);
    elementWiseMultiply(DUL2_raw,invsig_2,DUL2);
